#!/usr/bin/env python3
"""SSOT compiler: blueprint.yaml -> three artifacts, one compile, one source of truth.

  shadowrocket.conf  full config for Shadowrocket / Surge (iOS)
  clash-rules.yaml   full override config for mihomo-based client apps
  proxy-routes.list  bare rule list for a router that already owns its own config
                     (mihomo rule-provider: behavior classical, format text)

Runs in GitHub Actions, on an unrestricted network: fetches MetaCubeX .list files, converts them to
...
Why a third artifact: a router running OpenClash already has its own dns / tun / transparent-proxy
settings and its own lifeline rules, so handing it a *full* config would collide with those. It only
needs the domain list. Same compiled set, different packaging -- that is the point of the SSOT.
...
def is_bare_domain(s):
    # accept domain tokens incl. single-label TLD suffixes (+.io, +.alibaba); reject *, keyword:, regexp:, spaces
...
# ---------- domestic coverage set (to subtract mis-filed domestic domains from AI) ----------
dom_suf, dom_exa = set(), set()
for t, d in parse_domain(fetch(f"{UP}/geosite/cn.list"), "domestic"):
    (dom_suf if t == "DOMAIN-SUFFIX" else dom_exa).add(d)

def domestic_covers(domain):
    if domain in dom_exa or domain in dom_suf:
        return True
    parts = domain.split(".")
    return any(".".join(parts[i:]) in dom_suf for i in range(1, len(parts)))

# ---------- build groups ----------
counts = {}
groups = []  # each: (name, policy, [(TYPE,val), ...], is_ip)
# Local overlays stay their own rule-sets on the router side, so they are excluded from the bare list.
LOCAL = {g["name"] for g in BP["groups"] if g["source"]["kind"] == "local"}
for g in BP["groups"]:
    ...
    if src["kind"] == "local":
        ...
        if g.get("subtract_domestic"):
            before = len(rules)
            rules = [(t, d) for (t, d) in rules if not domestic_covers(d)]
            print(f"  {name}: subtracted {before-len(rules)} domestic domains")
...
# the AI group must contain zero domestic domains after subtraction
ai_rules = next(r for (n, p, r, ip) in groups if n == "ai")
leaked = [d for (t, d) in ai_rules if domestic_covers(d)]
if leaked:
    die(f"ai still contains domestic domains: {leaked[:10]}")
...
sr = []
sr.append("# Shadowrocket selective routing (default DIRECT) - AUTO-GENERATED by scripts/build.py, do not edit.")
sr.append("# " + hdr[0])
...
mihomo_head = f"""# mihomo selective routing (default DIRECT) - AUTO-GENERATED by scripts/build.py, do not edit.
# Client-facing override; a router should take proxy-routes.list instead (it owns its own config).
# {hdr[0]}
# No node baked in; include-all adopts whatever your subscription provides.
# Requires a mihomo-based core; rules are flat (portable to Stash / older cores). geo DB via the regional mirror below.
mode: rule
ipv6: false
unified-delay: true
tcp-concurrent: true
...
# ---------- emit proxy-routes.list (mihomo rule-provider, behavior: classical, format: text) ----------
# For a router that already has its own config and only needs the list. Two deliberate omissions:
#   1. no settings block  -- the router's own dns / tun / sniffer / lifeline rules stay in charge;
#   2. no policy column   -- the consumer decides, e.g. `- RULE-SET,routes,PROXY`.
# Entries are byte-identical to the PROXY entries in clash-rules.yaml (same compile, same upstream).
rp = ["# proxy-routes.list -- mihomo rule-provider (behavior: classical, format: text)",
      "# AUTO-GENERATED by scripts/build.py, do not edit.",
      f"# {hdr[0]}",
      "# Same compiled set as clash-rules.yaml, without the settings block and without the policy",
      "# column -- assign the policy at the consumer, e.g.  - RULE-SET,routes,PROXY",
      "# Local overlays (mine-proxy / mine-direct) are NOT included: they carry different policies",
      "# and are meant to be fetched as their own rule-sets straight from this repo."]
rp_n = 0
for name, pol, rules, is_ip in groups:
    if pol != "PROXY" or name in LOCAL:
        continue
    rp.append(f"# ---- {name} ----")
    for t, v in rules:
        rp.append(f"{t},{v}" + (",no-resolve" if is_ip else ""))
        rp_n += 1
rp.append("")
if rp_n < 1000:
    die(f"proxy-routes.list only got {rp_n} entries -- upstream groups look empty")
open(os.path.join(ROOT, "proxy-routes.list"), "w", encoding="utf-8").write("\n".join(rp))

print("OK:", " ".join(f"{k}={v}" for k, v in sorted(counts.items())))
print("upstream", sha, "->",
      "shadowrocket.conf", f"({len(sr)} lines)",
      "clash-rules.yaml", f"({len(mi)} lines)",
      "proxy-routes.list", f"({rp_n} entries)")
